...
function createMail($title, $content, $startTime, $finishTime, $deadline) { // メールを作成する関数
	$str = $title . "\n\n";
	$str .= "内容\n";
	$str .= $content . "\n\n";
	$str .= "実施日\n";
	...
	if (empty($errorMessage)) {
		...
		if (empty($_POST['allMember'])) {
			...
			// イベント開始時間 > イベント終了時間
				...
				$page = 0;
			// イベント日付 = イベント参加予約締切日 かつ イベント開始時間 <= イベント予約締切時間 > イベント開始時間
...
?>
							...
							<input type="time" id="deadlineTime" name="deadlineTime" value="<?php if (!empty($deadlineTime)) echo $deadlineTime; ?>"><br>

							<label for="mile1">マイルストン1:  イベント日</label>
							<input type="text" id="mile1" name="mile1Before" size="1" value="<?php if (!empty($mile1Before) || isset($mile1Before) && $mile1Before == 0) echo $mile1Before; ?>">
							日前の
							<input type="text" name="mile1Time" size="1" value="<?php if (!empty($mile1Time)) echo $mile1Time ?>">
							時<br>

							<label for="mile1">マイルストン2:  イベント日</label>
							<input type="text" id="mile2" name="mile2Before" size="1" value="<?php if (!empty($mile2Before) || isset($mile2Before) && $mile2Before == 0) echo $mile2Before; ?>">
							日前の
